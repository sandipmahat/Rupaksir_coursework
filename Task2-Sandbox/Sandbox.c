...
{
    int expected = VERDICT_NONE;
    return atomic_compare_exchange_strong(&S.verdict, &expected, want);
}
...
static void *sampler_thread(void *arg)
{
    ...
    while (!atomic_load(&S.stop_threads)) {
        ...
        if (sample_proc(S.child, &cpu_ms, &rss_kb) == 0) {
            ...
            if (S.cpu_limit_s > 0 &&
                (double)cpu_ms / 1000.0 >= S.cpu_limit_s) {
...
int main(int argc, char *argv[])
{
    ...
    if (S.child == 0) {
        ...
        /* Mirror the child's stdout/stderr into the log, so the log is
         * self-contained evidence of what the binary said as it died. */
        if (S.log) {
            int lfd = fileno(S.log);
